...
mixin GetReferenceMixin {
  static final Map<String, List<String>?> _referenceList = {};

  addReference<T>(State state, {String? tag}) {
    final newKey = GetInstance()._getKey(T, tag);
    String objc = identityHashCode(state).toString();
    if (_referenceList.containsKey(newKey)) {
      List<String> list = _referenceList[newKey] ?? [];
      list.add(objc);
    } else {
      _referenceList[newKey] = [objc];
    }
  }

  removeReference<T>(State state, {String? tag}) {
    final newKey = GetInstance()._getKey(T, tag);
    String objc = identityHashCode(state).toString();
    if (_referenceList.containsKey(newKey) && (_referenceList[newKey] ?? []).contains(objc)) {
      List<String> list = _referenceList[newKey] ?? [];
      list.remove(objc);
    }

    if (_referenceList.containsKey(newKey) && (_referenceList[newKey] ?? []).isEmpty) {
      _referenceList.remove(newKey);
...
class GetInstance with GetReferenceMixin{
